...
final class MVCModel
{
    ...
    private static final String DB_CREATE_QUERY = "CREATE TABLE " + MVCModel.TABLE_NAME + " (train_no text primary key, train_name text, source text, destination text, days_of_week text);";
    private static final String DB_CREATE_QUERY_STATION = "CREATE TABLE " + MVCModel.TABLE_NAME2 + " (station_code text primary key, station_name);";
    private static final String DB_CREATE_QUERY_TRAIN_STATION = "CREATE TABLE " + MVCModel.TABLE_NAME3 + " (train_no text, station_code text, time_arrival text, time_departure text);";
    ...
    public MVCModel(final Context ctx)
    {
        this.helper = new SQLiteOpenHelper(ctx, MVCModel.DB_NAME, null, MVCModel.DB_VERSION)
        {
            ...
            public void onCreate(final SQLiteDatabase db)
            {
                ...
                db.execSQL(MVCModel.DB_CREATE_QUERY_STATION);
                db.execSQL(MVCModel.DB_CREATE_QUERY_TRAIN_STATION);
            ...
            public void onUpgrade(final SQLiteDatabase db, final int oldVersion, final int newVersion)
            {
                ...
                db.execSQL("DROP TABLE IF EXISTS " + MVCModel.TABLE_NAME2);
                db.execSQL("DROP TABLE IF EXISTS " + MVCModel.TABLE_NAME3);
                str1 = "DDDDDDDDDDDDDATE...145: "+db.getPath();
//                db.getPath()
                this.onCreate(db);
            }
    ...
    public Cursor loadAllTrains()
    {
        ...
        final Cursor c = this.database.query(MVCModel.TABLE_NAME, new String[] { "train_no" , "train_name" , "source" , "destination" , "days_of_week"}, null, null, null, null, null); return c;
    }
...
//        final Cursor c = this.database.rawQuery("select t.train_no, t.train_name, t.source, t.destination, s.station_code, s.station_name from tblTrain t, tblStation s, tblTrainStation ts where t.source='pun' and ts.train_no=t.train_no and ts.station_code=s.station_code order by t.train_no",null);
    ...
    public void addStation(ContentValues data) {
        this.database.insert(MVCModel.TABLE_NAME2, null, data);
    }
    public void deleteStation(final String field_params)
    {
        this.database.delete(MVCModel.TABLE_NAME2, field_params, null);
        Log.v("DDDDDDDDATE","...144: "+str1);
    }
    public Cursor loadAllStations()
    {
        Log.d(MVCView.APP_TAG, "loadAllStations()");
        final Cursor c1 = this.database.query(MVCModel.TABLE_NAME2, new String[] { "station_code" , "station_name"}, null, null, null, null, "station_name",null);
        return c1;
    }
    public void addTrainStation(ContentValues data) {
        this.database.insert(MVCModel.TABLE_NAME3, null, data);
    }
    public void deleteTrainStation(final String field_params)
    {
        this.database.delete(MVCModel.TABLE_NAME3, field_params, null);
        Log.v("DDDDDDDDATE","...144: "+str1);
    }
    public Cursor loadAllTrainStations()
    {
        Log.d(MVCView.APP_TAG, "loadAllTrainStations()");
        final Cursor c1 = this.database.query(MVCModel.TABLE_NAME3, new String[] { "train_no" , "station_code", "time_arrival", "time_departure"}, null, null, null, null, null);
        return c1;
    }
}
